...
if(isset($_POST['submit'])) {    
    ...
    $subject_id = $crud->escape_string($_POST['abcd']); 
    ...
    elseif($imagefilesize<=256000)
      {
        ...
        {
          $img =  $candidate_id.'_'.$_FILES['upload']['name'];
        }
      }
      else
      {
        ...
        $result = $crud->execute("INSERT INTO `tbl_subject_info`(`subject_info_id`,`subject_id`,`subject_info`,`subject_image`, `created_by`, `modified_by`, `created_date`, `modified_date`, `currentStatus`) VALUES ('$subject_info_id','$subject_id','$subject_info','$img','GV','asis sarma',now(),now(), 'y')");
...
if(isset($_POST['update'])) {    
    $id = $crud->escape_string($_POST['id']);  
    $subject_id = $crud->escape_string($_POST['abcd']);
    $subject_info = $crud->escape_string($_POST['dept_name']);
    $targetdir = '../subject-info-images/';      
    ...
    if($imageFileType != 'jpg' &&  $imageFileType != 'png' && $imageFileType != 'jpeg')
    {
     ...
     echo '<script type="text/javascript">alert("Invalid file type!");</script>';
     echo '<script type="text/javascript">window.location.assign("../edit-subject-info.php");</script>';      
     die();   
