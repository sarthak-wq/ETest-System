...
if(isset($_POST['Submit'])) {    
    ...
    $candidate_id = CD.'-'.rand(10,999999);
...
if(isset($_POST['Update'])) {    
  ...
  if(!empty( $_FILES['img']['name'])) {
    ...
    $img = $crud->escape_string($_POST['previmg']);
      ...
      //insert data to database    
      $result = $crud->execute("UPDATE `tbl_candidate_registration` SET `first_name`='$fname',`last_name`='$lname',`gender`='$gender',`d_o_b`='$dob',`address`='$address',`mobile`='$mobile',`email`='$email',`password`='$pass',`avatar`='$img',`country`='$country',`state`='$state',`city`='$city',`package_type`='$fname',`package_expiry`=now(),`last_login_date`=now(),`modified_by`='$fname',`modified_date`=now()  WHERE id = '$candidate_id'");
